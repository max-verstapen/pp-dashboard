...
/**
 * Linking endpoint for email/Google account
 * 
 ...
 * 
 * Note: The frontend should check if the email is already linked to another user
 * before calling this endpoint. This endpoint just performs the linking.
 * 
 * - If user doesn't exist at address → returns 404
 * - Otherwise → links the email to the user
 */
export async function PUT(
	...
	const target = `${base}/users/${encodeURIComponent(address)}/email`;
